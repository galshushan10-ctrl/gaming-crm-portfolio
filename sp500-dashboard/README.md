...
הכי פשוט: GitHub Pages. ב-Settings → Pages של הריפו, בחר את הענף הזה ואת התיקייה
`/sp500-dashboard` (או פשוט את הענף עם root ל-`index.html` אם מעבירים את הקבצים לתיקיית שורש
נפרדת). זה דף סטטי לחלוטין — אין מגבלה בהוסטינג בכל שירות סטטי (Netlify, Vercel, GitHub Pages וכו').
